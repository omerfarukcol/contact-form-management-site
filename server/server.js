...
wss.on("connection", (socket) => {
  ...
  socket.on("message", (message) => {
    ...
    const responseMessage = `${messageData.name} --- ${messageData.message}`;
    socket.send(JSON.stringify({ message: responseMessage }));
  });
});
...
app.get("/api/messages-with-pagination", async (req, res) => {
  ...
  if (
    sortBy &&
    ["name", "gender", "creationDate", "country"].includes(sortBy)
  ) {
    messages.sort((a, b) => {
